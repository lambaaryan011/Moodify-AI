...
export default function MusicRecommendations() {
  const { recommendations, isLoading, setRecommendations, setCurrentTrack, setPlaying, setLoading } = useMusicStore();

  const handleMoodSelection = async (mood: string) => {
    // Set loading state to true while fetching recommendations
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex space-x-4 justify-center">
        {['Happy', 'Energetic', 'Calm', 'Focused', 'Sad', 'Neutral'].map((mood) => (
          <button
            key={mood}
